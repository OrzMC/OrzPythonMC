...
- PyPI 发布优先走 **Trusted Publisher(OIDC)**:`release.yml` 的 `pypi` job 带 **job 级** `permissions: {contents: read, id-token: write}`(job 级会覆盖工作流级,漏了 `id-token` 就换不到 token),且与 PyPI publisher 配置同为 `environment: release`;`scripts/publish_idempotent.sh` 用 **`--trusted-publishing always` 强制 OIDC**(`automatic` 在存在 token 时仍用 token,无法验证迁移是否成功),**没有 token 回退** —— OIDC 不可用就明确失败(迁移已完成,2026-09:两个包的预检均通过、`PYPI_API_TOKEN_*` secret 已删)。守卫测试禁止仓库里再出现 `UV_PUBLISH_TOKEN`/`PYPI_API_TOKEN`。PyPI 侧两个项目各自 Settings → Publishing → Add a new publisher(Owner/Repository/Workflow name=`release.yml`/Environment name=`release`)——**代码无法代办**;配好后跑 `gh workflow run release.yml --ref main -f check_oidc=true` 做预检(`release.yml` 的 `oidc-check` job,`check_oidc=true` 时其余发布 job 全部跳过):它用 `uv publish --dry-run --trusted-publishing always` **真的去换一次 OIDC token**(不上传文件),失败时还打印 PyPI 的原始响应(能点名不匹配的字段)。**预检必须在 `release.yml` 内**——PyPI 的 `Workflow name` 记的是文件名,放独立工作流文件里永远对不上(实测 `invalid-publisher`);本地跑必然报 `No OIDC token discovered`。**迁移已完成(2026-09)**:预检两包均通过,`pypi` job 无 `UV_PUBLISH_TOKEN`、脚本无 token 回退,两个 `PYPI_API_TOKEN_*` secret 已删除;2.3.0 是首个 OIDC-only 发版(日志 `已通过 OIDC(Trusted Publisher)发布 …`)。**PEP 740 attestation(provenance)**:uv **只上传** dist 里已存在的 `*.publish.attestation`(二进制里的文案:`Found attestation for distribution` / `By default, uv attempts to upload matching PEP 740 attestations`),**不生成**它们 —— 不签的话 PyPI 显示 `No provenance available`(2.3.0 就是如此)。所以 `pypi` job 在 `uv build` 之后、发布之前跑 `uvx "pypi-attestations==0.0.30" sign dist/*.whl dist/*.tar.gz`(PyPA 工具,用 job 的 GitHub OIDC 身份向 sigstore Fulcio/Rekor 签 bundle,依赖 `id-token: write`;版本钉死)。`oidc-check` job 跑同一套签名并断言「attestation 数量 == 发行件数量」,这样签名是否可用**不用等发版**就能验证。签名失败会让发版红(不静默降级),重跑该 job 是幂等的。**验证方式**:provenance **不在** PyPI 的 JSON API 里(2.3.0 的 `pypi/<pkg>/<ver>/json` 里没有 `provenance` 字段),只能看 `/integrity/<pkg>/<ver>/<file>/provenance`(200=有 / 404=无),或用 `uvx "pypi-attestations==0.0.30" verify pypi "pypi:<file>" --repository <构建产地仓库>`(对 `orzmc` 是 `https://github.com/OrzMC/OrzPythonMC`)。注意 `--repository` 是**构建产地仓库**(签名声明里的来源),不是发布者 —— 填错报 `provenance was signed by repository "X", expected "Y"`,这是校验在工作。**两个包名不同**——glob 用 `dist/<包>-*`(`orzmc-*` 不误匹配 `orzmc_app-*`)。
